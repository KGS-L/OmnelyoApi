...
def send_telegram_notification(message: str) -> None:
    ...
    if not config.TELEGRAM_BOT_TOKEN or not config.TELEGRAM_ADMIN_CHAT_ID:
        logger.warning("Notification Telegram non envoyée : token ou admin_chat_id manquant.")
        return
...
def get_remaining_slots(user_id: int | None = None) -> int:
    ...
    try:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT COUNT(*) FROM clips WHERE date(scheduled_publish_at) = date('now')"
            )
    ...
    while True:
        ...
        for slot in config.PUBLISH_SLOTS:
            ...
            with get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT COUNT(*) FROM clips WHERE scheduled_publish_at = ? AND status != 'failed'",
                    (candidate_str,)
                )
                count = cursor.fetchone()[0]
...
def process_source_video(source_video_id: int) -> None:
    ...
    try:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT source_url FROM source_videos WHERE id = ?", (source_video_id,))
            row = cursor.fetchone()
            if not row:
    ...
    send_telegram_notification(f"📥 <b>[Pipeline]</b> Démarrage du téléchargement pour la vidéo #{source_video_id}...")
    ...
    try:
        ...
        send_telegram_notification(f"❌ <b>[Erreur]</b> Échec du téléchargement de la vidéo #{source_video_id} : {e}")
        _mark_source_failed(source_video_id, str(e))
        return

    send_telegram_notification("✂️ <b>[Pipeline]</b> Analyse des scènes et découpage de la vidéo...")
    ...
    try:
        ...
        send_telegram_notification(f"❌ <b>[Erreur]</b> Analyse des scènes impossible pour la vidéo #{source_video_id} : {e}")
        _mark_source_failed(source_video_id, str(e))
    ...
    for i, (start_sec, end_sec) in enumerate(clip_ranges, start=1):
        ...
        send_telegram_notification(f"🎥 <b>[Clip {i}/{len(clip_ranges)}]</b> Traitement du clip ({duration:.1f}s)...")
        ...
        try:
            ...
            publish_slot = get_next_available_slot()
            ...
            send_telegram_notification(f"📤 <b>[Clip {i}/{len(clip_ranges)}]</b> Upload et programmation sur YouTube pour le {publish_slot.strftime('%d/%m à %H:%M')}...")
            ...
            try:
                ...
                youtube_uploader.update_video_thumbnail(yt_video_id, thumb_path)
            ...
            send_telegram_notification(f"❌ <b>[Erreur Clip {i}]</b> : {e}")
    ...
    _cleanup_files(temp_files)
    send_telegram_notification(f"🎉 <b>[Pipeline]</b> Traitement de la vidéo #{source_video_id} terminé !")
